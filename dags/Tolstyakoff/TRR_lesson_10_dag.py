...
def upload_data(week_start: str, week_end: str, **context):
    ...
    sql_query = f"""
        SELECT * FROM tolstyakoff_les_10_agg_weekly
    ...
    with pg.connect(
        ...
        s3_client.put_object(
            ...
            Key=f"tolstyakoff_{week_start}_{week_end}.csv"
        )
...
def combine_data(week_start: str, week_end: str, **context):
    ...
    sql_query = f"""
        INSERT INTO tolstyakoff_les_10_agg_weekly
        SELECT lti_user_id,
               attempt_type,
...
with DAG(
    dag_id="tolstyakoff_les_10_weekly",
    tags=['tolstyakoff', '10'],
    schedule='@daily',
    ...
    combine_data = PythonOperator(
        ...
        # },
        op_kwargs={
            'week_start': '{{ current_week_start(ds) }}', # 'week_start': '{{ current_week_start(ds, params.type) }}',
