...
def load_from_api(**context):
    ...
    with pg.connect(
        ...
        for el in data:
            ...
            row.append(passback_params.get('list_outcome_service_url'))

            cursor.execute("INSERT INTO table_for_dag_api VALUES (%s, %s, %s, %s, %s, %s, %s)", row)
        
        conn.commit()

with DAG(
    dag_id='load_from_api_4_1_ed',
    tags=['4', 'ed'],
    schedule='@daily',
