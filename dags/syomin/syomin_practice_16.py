...
from sensors.syomin_sqlsensor import SqlSensor
...
def save_row_data(data, week_start, week_end):
    ...
    with pg.connect(
        ...
        cursor = conn.cursor()
        cursor.execute(f"""
            DELETE FROM syomin_16_raw_table
        ...
        for record in data:
            ...
            cursor.execute(f"""
                INSERT INTO syomin_16_raw_table
...
FROM syomin_16_raw_table
WHERE week_start = %(week_start)s AND week_end = %(week_end)s;
"""

with DAG(
    dag_id='syomin_practice_16',
    tags=['16', 'syomin'],
    schedule='0 0 * * 1',
    ...
    sql_sensor = SqlSensor(
        ...
        sql="""
            ...
            from syomin_16_raw_table
            where created_at >= '{{ ds }}'
                and created_at < '{{ macros.ds_add(ds, 6) }}'
        """,
        mode='reschedule',
        poke_interval=300,
    )
    ...
    dag_start >> load_from_api >> sql_sensor >> agg_data >> dag_end